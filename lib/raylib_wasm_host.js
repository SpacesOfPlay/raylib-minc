...
//
// Natural-type extern ABI (lang c5a42db1): an import arg or result carries
// what its declared minc type maps to — i32, pointers, f32 and f64 are
// plain Numbers; only u32/u64/i64 are BigInt. The `env` runtime surface
// keeps its older all-i64 contract, and `math` keeps native f32/f64.
'use strict';
...
// WEBGL_debug_renderer_info is skipped: nothing here reads its strings and
// enabling it logs a deprecation warning in Firefox.
if (gl) (gl.getSupportedExtensions() || []).forEach(name => {
    if (name !== 'WEBGL_debug_renderer_info') gl.getExtension(name);
});

// A lost GL context (GPU recycle, driver reset, tab sleep) would otherwise
// show as a silent black canvas. Surface it; rlgl's GL objects don't
// survive a lost context, so recovery needs a reload.
let glLost = false;
if (canvas) {
...
const readCStr = (ptr) => {
    ...
    // slice, not subarray: TextDecoder rejects SharedArrayBuffer-backed views.
...
function rafLoop() {
    ...
    try {
        ...
        // Surface it — a console-only error leaves a frozen canvas.
...
RL.makeImports = function() {
    return {
        env: {
            ...
            // Byte count for a path; -1 when unknown. minc's file_size()
            // lowers to this on wasm.
            __minc_file_size: (pathPtr) => {
                const raw = readCStr(pathPtr);
                const norm = raw.replace(/^(\.\/)+/, '');
                const data = RL._vfs[norm] || RL._vfs[raw] || RL._vfs[norm.replace(/^.*\//, '')];
                return data ? BigInt(data.length) : -1n;
            },
            // Command-line args: none in the browser.
            get_argc: () => 0n,
            get_arg: (i) => 0n,
        ...
        app: {
            // Declared i32 (natural-type extern ABI, lang c5a42db1): Numbers.
            rl_host_width:  () => canvas.width,
            rl_host_height: () => canvas.height,
            rl_host_dpi_x1000: () => Math.round((window.devicePixelRatio || 1) * 1000),
        ...
        gl: (() => {
            ...
            const table = {
                ...
                glClearColor: v((r,g,b,a) => gl.clearColor(N(r),N(g),N(b),N(a))),
                glClearDepthf: v((d) => gl.clearDepth(N(d))),
                ...
                glBlendColor: v((r,g,b,a) => gl.blendColor(N(r),N(g),N(b),N(a))),
                glPixelStorei: v((p,x) => gl.pixelStorei(N(p),N(x))),
                glPolygonOffset: v((a,b) => gl.polygonOffset(N(a),N(b))),
                ...
                glTexParameterf: v((t,p,x) => gl.texParameterf(N(t),N(p),N(x))),
                glPixelStorei: v((p,x) => gl.pixelStorei(N(p),N(x))),
                ...
                // i32 result, so a Number; the u32 entry points below stay BigInt.
                glGetAttribLocation: (p,ptr) => gl.getAttribLocation(get(p),readCStr(ptr)),
                glBindAttribLocation: v((p,i,ptr) => gl.bindAttribLocation(get(p),N(i),readCStr(ptr))),
                glGetUniformLocation: (p,ptr) => { const loc=gl.getUniformLocation(get(p),readCStr(ptr)); return loc===null?-1:Number(put(loc)); },
                glUniform1i: v((l,x) => gl.uniform1i(get(l),N(x))),
                glUniform1f: v((l,x) => gl.uniform1f(get(l),N(x))),
                glUniform4f: v((l,x,y,z,w) => gl.uniform4f(get(l),N(x),N(y),N(z),N(w))),
                ...
                glGetString: (name) => 0,   // u8*, i.e. an i32 pointer
